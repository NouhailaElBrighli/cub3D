...
int render_next_frame(t_data *data, int keycode)
{
...
int key_press(int keycode, t_data *data)
{
	...
	else if (keycode == 119) // 13
	{
		data->move_up = 1;
		render_next_frame(data, keycode);
	}
	else if (keycode == 115) // 1
	{
		data->move_down = 1;
		render_next_frame(data, keycode);
	}
	else if (keycode == 100) // 0
	{
		data->move_right = 1;
		render_next_frame(data, keycode);
	}
...
void execution(t_data *data)
{
	...
	mlx_loop_hook(data->ptr->mlx, key_press, data);
